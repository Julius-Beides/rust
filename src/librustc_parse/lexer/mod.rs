...
impl<'a> StringReader<'a> {
    ...
    fn cook_lexer_token(&self, token: rustc_lexer::TokenKind, start: BytePos) -> TokenKind {
        match token {
            ...
            rustc_lexer::TokenKind::Literal { kind, suffix_start } => {
                ...
                let (kind, symbol) = self.cook_lexer_literal(kind, start, suffix_start);
    ...
    fn cook_lexer_literal(
        ...
        // The ' ' and " " literals validation is very different
        // from Int and Float, thus it's handled in a separate match expression.
        ...
        let (lit_kind, mode, prefix_len, postfix_len) = match kind {
            ...
            _ => unreachable!("Int and Float literals are handled in another match"),
        };
